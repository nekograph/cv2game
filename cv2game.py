...
class Enemyball():
    ...
    def __init__(self):
        ...
        for i in range(20):
            self.ball_angle = random.randint(-180,180)
            self.speed_x = math.cos(math.radians(self.ball_angle))*random.randint(3,8)
            self.speed_y = -2
            #xの位置,yの位置,x方向のスピード,y方向のスピード,角度
            self.balls.append([self.ball_x,self.ball_y,self.speed_x,self.speed_y,self.ball_angle])
        

    #壁が下にある時の反射
    def ball_reflection_down(self,ball,y): 
    ...
    #壁が上にある時の反射
    def ball_reflection_up(self,ball,y):
    ...
    #壁が左にある時の反射
    def ball_reflection_left(self,ball,x):
    ...
    #壁が右にある時の反射     
    def ball_reflection_right(self,ball,x):
    ...
    def move_ball(self,w,h):
        
        # ボール移動
        for ball in self.balls:
            ball[0] += round(math.cos(math.radians(ball[4]))*int(self.speed_x))
            ball[1] += round(math.sin(math.radians(ball[4]))*self.speed_y)

            self.ball_reflection_right(ball,0)
            self.ball_reflection_left(ball,w)
            self.ball_reflection_up(ball,0)
            self.ball_reflection_down(ball,h)
...
#敵の攻撃(ボール)を生成
enemy_ball = Enemyball()
print(enemy_ball.balls)
    ...
    if current == "title":
        ...
        for rule in text_how_to_play:
            if rule == "Attackは敵に攻撃":
                ...
                text_rule = font_asobi.render(rule,True,(255,255,100))
            else:
...
if camera_started == True:   
    ...
    while(True):
        ...
        #悪魔の画像をカメラに出す．
        if Devil.hp > 0 and You.hp >0:
            frame_resize = CvOverlayImage.overlay(frame_resize, devil,(int(w/2)-100,250))
        gray = cv2.cvtColor(frame_resize,cv2.COLOR_BGR2GRAY)
    
    #ステータス表示
        cv2.putText(frame_resize, f'You HP: {You.hp}', (10, 40), cv2.FONT_HERSHEY_SIMPLEX, 1, (30, 255, 0), 2)
        cv2.putText(frame_resize, f'You MP: {You.mp}', (10, 70), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 0), 2)
        cv2.putText(frame_resize, f'Cure rest: {You.Cure_remind}', (10, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,128,0), 2)
        cv2.putText(frame_resize, f'Enemy HP: {Devil.hp}', (w-500, 40), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
        
        #ステータスバー表示
        cv2.line(frame_resize,(300,30),(int(300+150*You.hp/You.max_hp),30),(0, 255, 0), 3)
        cv2.line(frame_resize,(300,60),(int(300+150*You.mp/100),60),(255, 255, 0), 3)
        cv2.line(frame_resize,(w-200,30),(int(w-200+150*Devil.hp/Devil.max_hp),30),(0, 0, 255), 3)
        ...
        if choice_limit +2<= choice_time - last_event_time <=enemy_turn_time and finish_flag == False:
            ...
            for ball in enemy_ball.balls:
                cv2.circle(frame_resize,(ball[0],ball[1]),enemy_ball.ball_size,(255,0,0),-1)
                enemy_ball.move_ball(w,h)
                
                # 線にぶつかるとダメージ
                for rect in face:
                    # 防御している場合
                    if rect[0]<= ball[0]<=rect[0]+rect[3] and rect[1]-5<=ball[1]<=rect[1]+5 and bougyo == True:
                        You.hp -= You.defence(Devil.power)
        ...
        if choice_time - last_event_time >= enemy_turn_time and You.hp > 0 and Devil.hp > 0:
            action_flag = False
            bougyo = False
